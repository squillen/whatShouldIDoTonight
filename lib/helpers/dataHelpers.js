...
export function slice (arr = [], start = 0, stop = 10) {
  return arr && Array.isArray(arr) ? arr.slice(start, stop) : arr
}

export function findCallOut (coll = [], check = {}) {
  function iterate (test) {
    for (let i = 0; i < coll.length; i++) {
      const item = coll[i]
      console.log('check.name :>> ', check.name)
      console.log('item.name :>> ', item.name)
      console.log('check.name === item.name :>> ', check.name === item.name)
      if (test(item)) return item
    }
    return null
  }
  if (coll && Array.isArray(coll)) {
    const foundShowCase = iterate((item) => item.showcase === true && check.name !== item.name)
    if (foundShowCase) return foundShowCase
    const foundNonSpotlight = iterate((item) => item.spotlight !== true && check.name !== item.name)
    if (foundNonSpotlight) return foundNonSpotlight
  }
  const randomIndex = Math.floor(Math.random() * (coll && coll.length) || 0)
