...
export default function displayContent ({ content = [], header, source, path, ref, homeRef }) {
  const href = `/${source}/all?category=${path || header.toLowerCase()}`
  return (
    content && Array.isArray(content) && content.length && content.length > 2
      ? (
          ...
          <div className={utilStyles.displayContentHeaderContainer}>
            <div className={utilStyles.leftHeaderContainer}>
              <Link href={href} as={href}>
                <a>
                  <h2 className={utilStyles.displayContentHeader} ref={ref}>{header}</h2>
